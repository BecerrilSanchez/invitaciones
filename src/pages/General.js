...
function General({ signOut, user }) {
  ...
  useEffect(() => {
    if (user) {
      ...
      console.log(
        "Usuario guardado en contexto y sessionStorage desde /General:",
        user
      );
          ...
          try {
            const response = await fetch(
              "https://gyela7sxvfg7hofofnpumj74ti.appsync-api.us-west-2.amazonaws.com/graphql",
              {
                method: "POST",
                headers: {
                  "x-api-key": "da2-pj4s5ve4afeprph43nrpr5mp6a", // API Key proporcionada en el curl
      ...
      console.log("Usuario no disponible en /General");
    }
  }, [user, setUser]);
...
export default withAuthenticator(General);
